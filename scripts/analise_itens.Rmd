...
diferenca_itens = item_contrato %>%
  mutate(ds_item = str_to_lower(gsub("[[:punct:]]", "", ds_item ))) %>% 
  group_by(ds_item) %>% 
  mutate(ocorrencias = n_distinct(id_orgao), 
         razao = vl_item_contrato / mean(vl_item_contrato)) %>%
  ungroup() %>% 
...
diferenca_itens %>% 
  ...
  # geom_count(alpha = .3) +
  geom_point(shape = 124, alpha = .7, color = "brown") + 
  scale_y_log10() + 
  coord_flip() + 
  labs(x = "", y = "Contratado / média do item")
```
